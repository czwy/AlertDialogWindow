...
namespace AlertDialogWindow.Toolkit
{
    ...
    internal class HtmlTextBlockUtils
    {
        // Title based conversion and replacement routines //
        ...
        {
            if ((HtmlTextBlockUtils.isNumber(c)) || (HtmlTextBlockUtils.isAlpha(c))) { return elementType.eText; }
            else
       ...
       public static String ExtractNextElement(ref string input, ref elementType element)
       {
        ...
        /// <summary>
        /// Convert Title Color(Blue) to System.Drawing.Color
        /// </summary>
        ...
        /// <summary>
        /// Convert Html Color(#FFFFFF) or Title Color(Blue) to System.Drawing.Color
        /// </summary>
        /// <param name="colorString"></param>
        /// <returns></returns>
        ...
        {            
            ...
            Int32 intPos = 0;
            while ((sizeStr.Length - 1 > intPos) && ((HtmlTextBlockUtils.isNumber(sizeStr[intPos + 1])) ||
            ...
            if (HtmlTextBlockUtils.isNumber(sizeStr[intPos]))
            {                 
...
//      		cm.Matrix40=cm.Matrix41=cm.Matrix42=.04f;
        ...
        /// </summary>
        public static bool Run2(string prog, string param, ref HtmlTextBlockProcessInfo pi)
        {
            //Int32 Infinite = Int32.MaxValue;
            //Int32 WAIT_OBJECT_0 = 0;
            
            Byte[] si = new Byte[128];
            if (pi == null) { pi = new HtmlTextBlockProcessInfo(); }

            Int32 returnCode = HtmlTextBlockHeader.CreateProcess(prog, param, IntPtr.Zero,
        ...
        {
        	return (HtmlTextBlockUtils.ExtractBefore(ref input, '{')).Trim();
        }
        private static string getDeclareSection(string input)
        {
        	return (HtmlTextBlockUtils.ExtractBetween(input, '{', '}')).Trim();
        }
        private static bool readTagName(string input, ref ArrayList cssStyleGroups)
        {
        	string k = HtmlTextBlockUtils.getHeaderSection(input);
        	
        	ArrayList cssTagNameList = new ArrayList();
        	string newTagName = HtmlTextBlockUtils.ExtractBefore(ref k, ',');
        	while (newTagName != "")
        	{
        		cssTagNameList.Add(newTagName);
        		newTagName = HtmlTextBlockUtils.ExtractBefore(ref k, ',');
        	}
        	...
        	for (Int32 i = 0; i < cssTagNameList.Count; i ++)
        	{
        		...
        		while (k.Trim() != "") 
        			newGroup.parentTagName.Add(HtmlTextBlockUtils.ExtractNextElement(ref k, ref e));
        ...
        private static bool readTagClass(string input, out string tagName, out string tagClass)
        {
        	...
        	tagName = HtmlTextBlockUtils.ExtractBefore(ref k, '.');
        	tagClass = k;
        ...
        private static bool readOneDeclare(string input, out string key, out string val)
        {
        	...
        	k = input; key = HtmlTextBlockUtils.ExtractBefore(ref k, ':').Trim();
        	k = input; val = HtmlTextBlockUtils.ExtractAfter(ref k, ':').Trim();
        ...
        {
        	string k = HtmlTextBlockUtils.getDeclareSection(input);
        	
        	string nextDeclare = HtmlTextBlockUtils.ExtractBefore(ref k, ';');
        	if (nextDeclare == "")
        	{
        		nextDeclare = k;
        		k = "";        	
        	...
        	while (nextDeclare != "")
        	{
        		...
        		if (key != "")
        			styles.Add(key, new HtmlTextBlockPropertyItemType(key, value));
        		
        		nextDeclare = HtmlTextBlockUtils.ExtractBefore(ref k, ';');
        		if (nextDeclare == "")
        		{
        			nextDeclare = k;
        			k = "";        	
        ...
        public static ArrayList DecodeCssStyle(string input)
        {
        	...
        	HtmlTextBlockUtils.readTagName(input, ref cssStyleGroups);
        	for (Int32 i = 0; i < cssStyleGroups.Count; i++)
        	{
        		...
        		newCssStyleType.tagName = (new HtmlTextBlockCssHeaderStyleType(curItem.styleTagName)).tagName;
        		if (newCssStyleType.tagName == "")
        			newCssStyleType.tagName = HtmlTextBlockCssHeaderStyleType.UnspecifiedTagName;
        ...
        private static bool matchHeader(HtmlTag input, HtmlTextBlockCssHeaderStyleType chs)
        {        	
        	return (((chs.tagName == HtmlTextBlockCssHeaderStyleType.UnspecifiedTagName) ||
        	          (input.Name == chs.tagName)) &&
        	          ((chs.tagClass == "") || (chs.tagClass == input["class"])) &&
        	          ((chs.tagID == "") || (chs.tagID == input["id"])));        	        	
        }
        private static bool haveOtherClassID(HtmlTag input, HtmlTextBlockCssHeaderStyleType chs)
        {
        	bool retVal = false;
...
//        }
        ...
        /// </summary>
        public static bool MatchCurrentTag(HtmlTag currentTag, string header)
        {
            HtmlTextBlockCssHeaderStyleType chs = new HtmlTextBlockCssHeaderStyleType(header);
