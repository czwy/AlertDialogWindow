...
namespace AlertDialogWindow.Toolkit
{
    ...
    /// </summary>
    #region Enums Types
    internal enum loadType { ltString, ltFile, ltWeb, ltWebNoCache }                  //Define where to load from.
    //public enum threeSide { _default=0, _left, _top, _right }
    internal enum fourSide { _default = 0, _left, _top, _right, _bottom }               //Define Left, Top, Right and Bottom
    internal enum hAlignType { Unknown, Left, Right, Centre }                         //Define visible object horizontal hAlign
    internal enum vAlignType { Unknown, Top, Bottom }                                 //Define visible object verticial hAlign
    internal enum formMethodType { Default, Get, Post }                               //Define form action
    internal enum tagStatusType { Normal, Focused, Active }                           //Define state of a visile tag
    internal enum selectInfoPairs { sStart, sEnd }                                    //Define Start and End of SelectInfo
    internal enum parseMode { Text = 0, Html, BBCode }                          		//Parse html or bbcode

    internal enum HTMLFlag
    {
        TextFormat, Element, Dynamic, Table,
        Controls, Search, Xml, Region, Variable, None
    }                                                         //Define tag type in BuiltInTags
    internal enum aTextStyle { isNormal, isSubScript, isSuperScript }                 //Define text style
    internal enum loadStatus { Idle, Load, Update, Draw, Overlay }                    //Define what is miniHtml doing
    internal enum elementType { eSpace, eText, eSymbol, eId, eClass, eStyle, eDash }  //Define type of a char
    internal enum symbolType { Reserved, European, Symbol, Scientific, Shape }        //Define symbol type in BuiltInSymbols
    internal enum textTransformType { None, Uppercase, Lowercase, Capitalize }        //Define how to transofm a text
    internal enum positionStyleType { Static, Relative, Absolute, Fixed, Inherited }  //Define how to allocate a tag
    internal enum borderStyleType                                                     //Define a list of border style 
    {
        None, Dotted, Dashed, Solid, Double, Groove, Ridge,
        Inset, Outset, Inherit
    }
    internal enum bulletStyleType                                                     //Define a list of bullet style 
    {
        None, Circle, Square, Decimal, UpperAlpha, LowerAlpha,
        UpperRoman, LowerRoman
    }
    internal enum variableType { Number, Alpha, String, Formated, Paragraph }         //Define variableType of variableTag (for search text)
    #endregion
    ...
    //internal class KeyValuePair { public String key, value; }
    //internal class FontStyleSet { public bool bold, italic, regular, strikeout, underline;}
    ...
    internal class Defines
    {
        ...
        public static HTMLTagInfo[] BuiltinTags = new HTMLTagInfo[51] {   
          #region Built in tag list
           ...
           // 06
           // 05 A HtmlTag, Input
           // 04 Title formating tags (B, strong, U, S, I, FONT, sub, sup), 
           // 03
           // 02
           // 01 Unknown Tags, script
           // 00 Title, hr, user, Img, Dynamic, BR, Meta, Binding
