...
//            HtmlTagNode newNode = new HtmlTagNode(
        ...
        {
            string beforeTag=string.Empty, afterTag= string.Empty, tagName= string.Empty, tagVar= string.Empty;
            readNextTag(s, ref beforeTag, ref afterTag, ref tagName, ref tagVar);
            
            if (beforeTag?.Length>0)
            	addTag(new HtmlTag(beforeTag));   		//Title
            if (tagName?.Length > 0)
